...
def initGame():
    ...
    aircraft = pygame.image.load('./image/spaceship.png')
    background1 = pygame.image.load("./image/fly_background.png")
    background2 = background1.copy()  # 배경이 움직이도록 이미지 복사본을 원본 다음으로 좌표 지정

    Shotship = pygame.image.load("./image/Shotship.png")

    sonic1 = pygame.image.load("./image/sonic1.png")
    sonic2 = pygame.image.load("./image/sonic2.png")
...
class TestAdd(unittest.TestCase):
    def test_textObj(self):
        self.assertEqual(textObj('crashed', 'freesansbold.ttf'))

    def test_dispMessage(self):
        self.assertEqual(dispMessage('crashed'))

    def test_crash(self):
        self.assertEqual(crash())

    def test_drawObject(self):
        self.assertEqual(drawObject(aircraft, 30, 30))

    def test_runGame(self):
        self.assertEqual(runGame())

    def test_initGame(self):
        self.assertEqual(initGame())
